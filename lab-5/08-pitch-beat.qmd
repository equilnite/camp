		...
		<title>Unit 2 Lab 1: Song Player, Activity 8</title>
	</head>
...
}" title="/img/U2/lab01/play-song-definition.png" alt="play song (pitches) {
	for each (pitche) in (pitches) {
		play ♪ pitch: (pitch) beats: (0.5)
	}
}" />
...
}" title="play song with pitches and beats (song) {
	for each (♪) in (song) {
		play ♪ pitch: (pitch from ♪ (♪)) beats: (beats from ♪ (♪))
	}
}" />
